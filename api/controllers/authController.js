...
try {
    ...
    const token = jwt.sign({id:exist._id,name:exist.username,email:exist.email},process.env.JWT_SECRET)   
    const {password,...other} = exist._doc
    res.cookie("accessToken", token, {httpOnly: true ,sameSite:'none',secure:true }).status(201).json(other)
...
export const logout = async (req,res,next) =>{
    ...
    res.clearCookie('accessToken',{sameSite:"none",secure:true}).status(200).send('user logged out')
